...
const props = defineProps<{ data: KdpPartner[] }>()
</script>

<template>
  <UCard>
    <div v-if="props.data" class="flex flex-col gap-8">
      <UAccordion :items="props.data.map((p, i) => ({ label: p.title, slot: `partner-${i}` }))">
        <template v-for="(item, key) in props.data" :key="key" #[`partner-${key}`]>
